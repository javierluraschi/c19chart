...
highlight_since_start %>%
  ggplot(aes(x = since_start, y = total_percent, colour = country)) +
    ...
    annotate("text", x=71.3, y = max(deaths_influenza$total_percent) + 0.00006, label = "influenza (93/day US)", size = 3.5, colour="grey70") +
    geom_line(data = deaths_clrd, alpha = 0.3, colour="grey70", linetype="dashed") +
    annotate("text", x=63.9, y = max(deaths_clrd$total_percent) + 0.0003, label = "chronic lower respiratory diseases (438/day US)", size = 3.5, colour="grey70") +
    geom_line(data = deaths_heart, alpha = 0.3, colour="grey70", linetype="dashed") +
    annotate("text", x=69.7, y = max(deaths_heart$total_percent) + 0.001, label = "heart disease (1772/day US)", size = 3.5, colour="grey70") +
